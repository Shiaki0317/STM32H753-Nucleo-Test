...
static err_t low_level_output(struct netif *netif, struct pbuf *p)
{
  ...
  for(q = p; q != NULL; q = q->next)
  {
    ...
    /* Make CPU writes visible to Ethernet DMA when D-Cache is enabled. */
...
/**
  * @brief  Notify the main loop that Ethernet DMA reception completed.
  * @param  handlerEth Ethernet HAL handle
  * @retval None
  */
...
/**
  * @brief  Wake the receive path when DMA reports no receive buffer.
  * @param  handlerEth Ethernet HAL handle
  * @retval None
  */
...
/**
  * @brief  Atomically consume the Ethernet DMA receive notification.
  * @retval 1 when received data may be pending, otherwise 0
  */
...
/* USER CODE BEGIN HAL ETH RxLinkCallback */
  ...
  *ppEnd  = p;
  ...
  /* Invalidate stale cache lines only when the D-Cache is enabled. */
  if ((SCB->CCR & SCB_CCR_DC_Msk) != 0U)
  {
